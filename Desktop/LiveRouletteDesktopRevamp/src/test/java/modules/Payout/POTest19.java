...
public class POTest19 extends Payout implements PayoutTestCase {

    private double bet, payout;
    private final int testCase = 19, payoutOdds = 11;
    private final int[][][] betOptionList = Arrays.copyOf(Variables.ZERO_CORNER_LIST, Variables.ZERO_CORNER_LIST.length);
    ...
    public void placeBettingOption() {
        ...
        for (int[][] betOption : betOptionList)
            EventHandler.click(XPath.GameTable.BettingOption.Option.getZeroCorner(betOption));
    }
    ...
    public void checkBettingChip() {
        if (isZeroCornerPlaced) {
            int i = 0;
            for (int[][] betOption : betOptionList)
                chipValueList[i++] = getChipValue(XPath.GameTable.BettingOption.Chip.getZeroCorner(betOption));
        }
    }
    ...
    public void saveTestCase(String[] roundResult, List<PayoutTestCase> payoutTestCaseList) {
        if (isZeroCornerPlaced) {
            ...
            payoutTestCaseList.removeIf(payout -> payout instanceof POTest19 && payout.getTestCase() == testCase);
        }
    }
