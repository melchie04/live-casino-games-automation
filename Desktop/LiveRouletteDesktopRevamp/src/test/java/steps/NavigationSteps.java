...
public class NavigationSteps {
    ...
    public void iEnterTheLiveRouletteDesktopRevampThumbnail() {
        Navigator navigator = () -> {
            Variables.productName = "Roulette";
            // Go to game lobby page
            if (ConditionHandler.isDisplayed(XPath.LiveCasino.LiveRouletteDesktopRevamp, 5)) {
                EventHandler.click(XPath.LiveCasino.LiveRouletteDesktopRevamp);
                NavigationHandler.switchToAnotherWindow();
            }
            // Go back to the game lobby page if there is a previous session
            if (ConditionHandler.isUrlContains("/rouletteDesktop", 5))
        ...
        retrySteps(navigator, "Enter The Live Roulette Desktop Revamp Thumbnail.");
    }
